...
[CmdletBinding()]
Param (
	[Parameter(Mandatory = $false, ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true, Position =
			   0)]
...
Begin
{
	...
	function CheckIfAdmin
	{
		...
		$global:token = ([System.Security.Principal.WindowsIdentity]::GetCurrent()).token
	...
	function ResolveTargetName
	{
		...
		try
		{
			...
			$iptostring = ([System.Net.Dns]::GetHostAddressesAsync($TargetName).GetAwaiter().GetResult()).ipaddresstostring
	...
	function PingTest
	{
		...
		Write-Progress -Activity "Test-NetConnection :: $TargetIPAddress" -Status "Ping/ICMP Test"
		-CurrentOperation "Waiting for echo reply" -SecondsRemaining -1 -PercentComplete -1
	...
	##Output: Array of IP addresses representing the traced route. The message from the ping reply status is 
	emmited, if there is no response.
	function TraceRoute
	{
		param ($TargetIPAddress,
			$Hops)
		...
		do
		{
			try
			{
				...
				write-progress -CurrentOperation "TTL = $CurrentHop" -Status "ICMP Echo Request (Max TTL = $Hops)"
				-Activity "TraceRoute" -PercentComplete -1 -SecondsRemaining -1
				$PingReplyDetails = $Ping.SendPingAsync($TargetIPAddress, 4000, $DataBuffer,
					$PingOptions).GetAwaiter().Getresult()
		...
		{
			$OutputString = "Trace route to destination " + $TargetIPAddress + " did not complete. Trace 
terminated :: " + $ReturnTrace[-1]
	...
	function TestTCP
	{
		...
		Write-Progress -Activity $ProgressString -Status "Attempting TCP connect" -CurrentOperation "Waiting for 
response" -SecondsRemaining -1 -PercentComplete -1
	...
	function ResolveRoutingandAdapterWMIObjects
	{
		...
		try
		{
			$TestNetConnectionResult.SourceAddress, $TestNetConnectionResult.NetRoute = Find-NetRoute
			-RemoteIPAddress $TestNetConnectionResult.RemoteAddress -ErrorAction SilentlyContinue
			$TestNetConnectionResult.NetAdapter = $TestNetConnectionResult.NetRoute | Get-NetAdapter
			...
			$TestNetConnectionResult.InterfaceDescription =
			$TestNetConnectionResult.NetAdapter.InterfaceDescription
		}
		catch
		{
	...
	function ResolveDNSDetails
	{
		...
		$TestNetConnectionResult.DNSOnlyRecords = @(Resolve-DnsName $ComputerName -DnsOnly -NoHostsFile -Type
			A_AAAA -ErrorAction SilentlyContinue | where-object { ($_.QueryType -eq "A") -or ($_.QueryType -eq "AAAA") })
		$TestNetConnectionResult.LLMNRNetbiosRecords = @(Resolve-DnsName $ComputerName -LlmnrNetbiosOnly
			-NoHostsFile -ErrorAction SilentlyContinue | where-object { ($_.QueryType -eq "A") -or ($_.QueryType -eq "AAAA") })
		$TestNetConnectionResult.BasicNameResolution = @(Resolve-DnsName $ComputerName -ErrorAction
			SilentlyContinue | where-object { ($_.QueryType -eq "A") -or ($_.QueryType -eq "AAAA") })
		
		$TestNetConnectionResult.AllNameResolutionResults = $Return.BasicNameResolution + $Return.DNSOnlyRecords
		+ $Return.LLMNRNetbiosRecords | Sort-Object -Unique -Property Address
		return $TestNetConnectionResult
	}
	...
	function ResolveNetworkSecurityDetails
	{
		...
		$TestNetConnectionResult.IsAdmin = CheckIfAdmin(tnc $TestNetConnectionResult -TraceRoute).traceroute | % { Resolve-DnsName $_ -Type PTR -ErrorAction SilentlyContinue }
		$dnscheck = 
		$NetworkIsolationInfo = Invoke-CimMethod -Namespace root\standardcimv2 -ClassName MSFT_NetAddressFilter
		-MethodName QueryIsolationType -Arguments @{
		...
		{
			$TestNetConnectionResult.MatchingIPsecRules = Find-NetIPsecRule -RemoteAddress
			$TestNetConnectionResult.RemoteAddress - RemotePort $TestNetConnectionResult.RemotePort -Protocol TCP -ErrorAction
			SilentlyContinue
		}
	...
	function DiagnoseRouteSelection
	{
		...
		{
			Write-Progress -Activity "Test-NetConnection :: $($RouteDiagnostics.RemoteAddress)" -Status
			"RouteDiagnostics" - CurrentOperation "Starting Route Event Tracing" -SecondsRemaining -1 -PercentComplete -1
			...
			$TraceResults = netsh trace start tracefile=$LogFile provider=Microsoft-Windows-TCPIP
			keywords=ut:TcpipRoute report=di perfmerge=no correlation=di
		}
		
		Write-Progress -Activity "Test-NetConnection :: $($RouteDiagnostics.ComputerName)" -Status
		"RouteDiagnostics" - CurrentOperation "Resolving name" -SecondsRemaining -1 -PercentComplete -1
		...
		Write-Progress -Activity "Test-NetConnection :: $($RouteDiagnostics.RemoteAddress)" -Status
		"RouteDiagnostics" - CurrentOperation "Finding route" -SecondsRemaining -1 -PercentComplete -1
		
		try
		{
			$RouteDiagnostics.SelectedSourceAddress, $RouteDiagnostics.SelectedNetRoute = `
			Find-NetRoute -RemoteIPAddress $RouteDiagnostics.RemoteAddress -LocalIPAddress
			$RouteDiagnostics.ConstrainSourceAddress - InterfaceIndex $RouteDiagnostics.ConstrainInterfaceIndex -ErrorAction
			SilentlyContinue
			
			$RouteDiagnostics.OutgoingNetAdapter = $RouteDiagnostics.SelectedNetRoute | Get-NetAdapter
			-IncludeHidden -ErrorAction SilentlyContinue
			...
			$RouteDiagnostics.OutgoingInterfaceDescription =
			$RouteDiagnostics.OutgoingNetAdapter.InterfaceDescription
		}
		catch
		{
		...
		{
			Write-Progress -Activity "Test-NetConnection :: $($RouteDiagnostics.RemoteAddress)" -Status
			"RouteDiagnostics" - CurrentOperation "Parsing Route Events" -SecondsRemaining -1 -PercentComplete -1
			...
			##Get route selection events: [IpRouteSelection (task:1378) and IpRouteBlocked (task:1379)] containing 
			the remote IP
			$RouteEvents = Get-WinEvent -Path $LogFile -Oldest | Where-Object {
				(($_.task -eq "1379") -or ($_.task
						-eq "1378")) -and (($_.Message -ne $null) -and $_.Message.Contains("$($RouteDiagnostics.RemoteAddress) "))
			}
			...
			$SrcAddrEvents = Get-WinEvent -Path $LogFile -Oldest | Where-Object {
				($_.task -eq "1326") -and
				(($_.Message -ne $null) -and $_.Message.Contains("$($RouteDiagnostics.RemoteAddress) "))
			}
			...
			##Get destination address selection events: [IpSortedAddressPairs (task:1327)] containing the resolved 
			IPs
			$DstAddrEvents = Get-WinEvent -Path $LogFile -Oldest | Where-Object {
				($_.task -eq "1327") -and
				((($Message = $_.Message) -ne $null) -and (@(($RouteDiagnostics.ResolvedAddresses | ForEach-Object
								{ $_.IPAddressToString }) | where { $Message.Contains("$_)") }) -ne $null))
			}
...
Process
{
	switch ($PSCmdlet.ParameterSetName)
	{
		...
		default
		{
			...
			$AttemptTcpTest = ($PSCmdlet.ParameterSetName -eq "CommonTCPPort") -or ($PSCmdlet.ParameterSetName -eq
				"RemotePort")
			if ($AttemptTcpTest)
			{
				...
				{
					$Return.TcpTestSucceeded = TestTCP -TargetIPAddress $Return.ResolvedAddresses[$Iter] -TargetPort
					$Return.RemotePort
					##Output a warning message if the TCP test didn't succeed
					if (-not $Return.TcpTestSucceeded)
					{
						Write-Warning "TCP connect to ($($Return.ResolvedAddresses[$Iter]) : $($Return.RemotePort)) 
failed"
					}
					$Iter++
				}
			...
			if ($AttemptPingTest)
			{
				...
				while (($Iter -lt $Return.ResolvedAddresses.Count) -and (-not $Return.PingSucceeded))
				{
					...
					{
						$Return.PingSucceeded = ($Return.PingReplyDetails.Status -eq
